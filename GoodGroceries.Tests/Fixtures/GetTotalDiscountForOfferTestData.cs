...
namespace GoodGroceries.Tests.Fixtures
{
    public class GetTotalDiscountForOfferTestData : IEnumerable<object[]>
    {
        ...
        {
            var batch1 = new List<(Product, int)> { (_fixtures.Products[0], 3) };

            var batch2 = new List<(Product, int)> { (_fixtures.Products[0], 1), (_fixtures.Products[1], 1) };

            var batch3 = new List<(Product, int)> { (_fixtures.Products[2], 4) };
